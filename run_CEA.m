% run_CEA

% runs NASA CEA for a given set of variable parameters

% Inputs: Pc, OF, Pamb, A_e, eps (Ae/At), Propellant Mass
% Outputs: 
    % Prop: C*, C_tau, A_t, mdot, Thrust, Isp, Burn Time
    % Abl: T0 stagnation temp, Pr Prandtl number, gamma, Cp
        % [injector, nozzle entrance]


function [Prop, Abl] = run_CEA(Prop, params, eth_ratio)

    Engine = CEA( ...
        'problem','rocket', ...
        'equilibrium','fac', ...
        'ac/at',params.ac_at, ...
        'o/f',Prop.OF, ...
        'p(psi)', Prop.Pc, ...
        'supersonic', Prop.eps, ...
        'reactants', ...
            'fuel','C2H5OH(L)', ...
                'wt%',eth_ratio*100, ...
                't(k)',params.T_fuel, ...
            'fuel','H2O(L)', ...
                'wt%',(1-eth_ratio)*100, ...
                't(k)', params.T_fuel, ...
            'oxid','O2(L)', ...
                'wt%',100, ...
                't(k)', 90.17, ... % use instead of params.T_ox to prevent warnings, CEA was using this value anyways
        'output','transport', ...
        'mks', ...
        'end' ... %, 'screen' ...
    );

    Abl.T0 = Engine.output.eql.temperature(1:2); % K
    Abl.Pr = Engine.output.eql.prandtl.eql(1:2);
    Abl.gamma = Engine.output.eql.gamma(1:2);
    Abl.Cp = Engine.output.eql.cp(1:2); % J/kg-K

    % Calculate C_star

    C_star_SI = Engine.output.eql.cstar(1); % m/s
    Prop.C_star = C_star_SI * 3.28084; %m/s to ft/s

     % Calculate C_tau

    Prop.Ctau = Engine.output.eql.cf(end); 

    % Calculate Exit Pressure

    Prop.Pe = Engine.output.eql.pressure(end) * 14.5038; % bar to psi

    % Calculate C_tau at Vaccuum

    Prop.Ctau_vac = Prop.Ctau + Prop.eps * (Prop.Pe / Prop.Pc); 

    % Calculate throat area

    Prop.A_t = params.A_e / Prop.eps; % in^2
    At_SI = Prop.A_t * 0.00064516; % in^2 to m^2

    % Calculate mdot

    Pc_pascals = Prop.Pc * 6894.76; % psi to Pa for mdot
    mdot_SI = (At_SI * Pc_pascals) / (C_star_SI * params.Cstar_eff); %kg/s
    Prop.mdot = mdot_SI * 2.20462; % kg/s to lbm/s
    Prop.mdot_fuel = Prop.mdot / (1 + Prop.OF); %lbm/s
    Prop.mdot_ox = Prop.mdot - Prop.mdot_fuel; % lbm/s

    % Calculate burn time

    Prop.t_b = Prop.prop_mass / Prop.mdot; % s 

    % Calculate Ctau at set ambient pressure

    Ctau_ref = Prop.Ctau_vac - Prop.eps * (params.P_amb / Prop.Pc);

    % Calculate thrust for Isp

    Thrust_ref_SI = mdot_SI * C_star_SI * params.Cstar_eff * Ctau_ref * params.Ctau_eff; % N
    Prop.Thrust = Thrust_ref_SI * 0.224809; % N to lbf

    % Calculate specific impulse

    Prop.Isp = Prop.Thrust / (Prop.mdot); % s
end