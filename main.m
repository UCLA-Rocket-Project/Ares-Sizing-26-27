...
df_filtered = readtable(filtered_csv_path);

if isempty(df_filtered)
    fprintf('No valid combinations found, check filters & ranges\n')

else
    [max_apogee, opt_idx] = max(df_filtered.apogee);
    ...
    log = sprintf([...
        ...
        it, height(df_filtered), elapsed_time, ...
        max_apogee, ...
        df_filtered.prop_mass(opt_idx), df_filtered.thrust(opt_idx), ...
        df_filtered.t_b(opt_idx), df_filtered.mdot(opt_idx), ...
        df_filtered.OF(opt_idx), df_filtered.Pc(opt_idx), ...
        df_filtered.tank_press(opt_idx), df_filtered.Isp(opt_idx), ...
        df_filtered.dry_mass(opt_idx), df_filtered.fuel_tank_length(opt_idx), ...
        df_filtered.ox_tank_length(opt_idx), df_filtered.vehicle_length(opt_idx));
...
if ~isempty(df_filtered) 
    figure;
    plot(df_filtered.vehicle_length, df_filtered.apogee, '.');
    xlabel('Vehicle Length (in)');
    ylabel('Apogee (ft)');

    figure;
    plot(df_filtered.prop_mass, df_filtered.apogee, '.');
    xlabel('Prop mass (lb)');
    ylabel('Apogee (ft))');
